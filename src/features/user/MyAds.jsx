...
const MyAds = () => {
    const { user } = useAuth();
    const { ads, refreshAds } = useAdsFetchByUser(user.id);


    return (
        <section id="my-ads-section" className="max-w-[1000px] mx-auto mt-15">
            <h2 className="text-4xl text-center font-bold text-slate-700 pb-4">
                My Ads
            </h2>

            {ads.map((ad) => (
                <UserAdCard key={ad.id} refreshAds={refreshAds} ad={ad} />
            ))}
